...
namespace _460ASDAL
{
    public class DAL460AS_Usuario : DAL_Abstract
    {
        ...
        {
            string consulta = "INSERT INTO USUARIO_460AS (DNI_460AS, Nombre_460AS, Apellido_460AS, Login_460AS, Password_460AS, CodPerfil_460AS, Telefono_460AS, Bloqueado_460AS, Activo_460AS, Contador_460AS, UltimoIntento_460AS, Idioma_460AS)" +
                              "VALUES (@DNI_460AS, @Nombre_460AS, @Apellido_460AS, @Login_460AS, @Password_460AS, @CodPerfil_460AS, @Telefono_460AS, @Bloqueado_460AS, @Activo_460AS, @Contador_460AS, @UltimoIntento_460AS, @Idioma_460AS)";
            using (SqlConnection conexion = new SqlConnection(cx))
            {
                ...
                using (SqlCommand comando = new SqlCommand(consulta, conexion))
                {
                    ...
                    comando.Parameters.AddWithValue("@CodPerfil_460AS", usuario.Rol_460AS?.Codigo_460AS ?? (object)DBNull.Value);
        ...
        public void ActualizarUsuario_460AS(Usuario_460AS usuario)
        {
            string consulta = "UPDATE USUARIO_460AS SET Nombre_460AS = @Nombre_460AS, Apellido_460AS = @Apellido_460AS, Login_460AS = @Login_460AS, " +
                     "Password_460AS = @Password_460AS, CodPerfil_460AS = @CodPerfil_460AS, Telefono_460AS = @Telefono_460AS, Bloqueado_460AS = @Bloqueado_460AS, Activo_460AS = @Activo_460AS, Contador_460AS = @Contador_460AS, UltimoIntento_460AS = @UltimoIntento_460AS, Idioma_460AS = @Idioma_460AS " +
        ...
        {
            List<Usuario_460AS> lista = new List<Usuario_460AS>();

            using (SqlConnection con = new SqlConnection(cx))
            {
                SqlCommand cmd = new SqlCommand("SELECT * FROM USUARIO_460AS", con);
                con.Open();
                ...
                using (SqlDataReader reader = cmd.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        Perfil_460AS perfil = new Perfil_460AS
                        {
                            Codigo_460AS = reader["CodPerfil_460AS"].ToString()
                        };

                        Usuario_460AS user = new Usuario_460AS(
                            reader["DNI_460AS"].ToString(),
                            reader["Nombre_460AS"].ToString(),
                            reader["Apellido_460AS"].ToString(),
                            reader["Login_460AS"].ToString(),
                            reader["Password_460AS"].ToString(),
                            perfil,
                            Convert.ToInt32(reader["Telefono_460AS"]),
                            Convert.ToBoolean(reader["Bloqueado_460AS"]),
                            Convert.ToBoolean(reader["Activo_460AS"]),
                            Convert.ToInt32(reader["Contador_460AS"]),
                            Convert.ToDateTime(reader["UltimoIntento_460AS"]),
                            reader["Idioma_460AS"].ToString()
                        );
